...
result<ref<VideoReader>> VideoReader::Open(const std::filesystem::path& path) {
    ...
    if (IsCameraIndex(pathStr)) {
        ...
        return ok(createRef<VideoReader>(std::move(cap), std::numeric_limits<u64>::max(), fps, source));
    }
    ...
    if (!cap.isOpened())
        return err(ErrorCode::CREATE_PIPELINE_FAILED, "Failed to open video: " + pathStr);
...
result<Reader::FrameData> VideoReader::Next() {
    if (!HasNext())
        return err(ErrorCode::INVALID_ARGUMENT, "No more frames to read");
    ...
    if (frame.empty()) {
        ...
        return err(ErrorCode::UNKNOWN_ERROR, "Failed to read frame at index " + std::to_string(mCurrentIndex));
    }
...
bool VideoReader::HasNext() const {
    ...
    if (mSource == VideoSourceType::Camera || mSource == VideoSourceType::Stream)
        return true;
